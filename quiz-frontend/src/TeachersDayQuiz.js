...
function TeachersDayQuiz({ onBack }) {
  ...
  const loadQuestions = useCallback(async () => {
    ...
    try {
      console.log('Loading Teacher\'s Day questions...');
      const questionPromises = teacherQuestionIds.map(id => 
  ...
  if (loading) {
    return <div className="teachers-quiz-loading">Loading Teacher's Day quiz...</div>;
  }

  if (questions.length === 0) {
    return <div className="teachers-quiz-loading">No questions available. Please try again later.</div>;
  }

  if (showResult) {
    return (
      <div className="teachers-quiz-result">
        <div className="result-content">
          <h1>📚 Teacher's Day Quiz Complete!</h1>
          <div className="score-display">
            <span className="score">{score}</span>
            <span className="total">/ {questions.length}</span>
          </div>
          <p className="score-message">
            {score >= 25 ? "Outstanding! You're a true admirer of teachers and education!" :
             score >= 20 ? "Excellent! You have great knowledge about teachers and their contributions." :
             score >= 15 ? "Good work! You appreciate the importance of teachers in society." :
             score >= 10 ? "Nice effort! Keep learning about the noble profession of teaching." :
             "Every teacher was once a student. Keep exploring and learning!"}
          </p>
  ...
  if (!currentQuestion) {
    return <div className="teachers-quiz-loading">Loading question...</div>;
  }

  return (
    <div className="teachers-day-quiz">
      ...
      <div className="question-container">
        {currentQuestion.imageId && (
          <div className="question-image">
            <img src={currentQuestion.imageId} alt="Teacher's Day" />
          </div>
        )}
...
export default TeachersDayQuiz;
