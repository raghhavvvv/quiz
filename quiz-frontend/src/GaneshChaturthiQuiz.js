...
import './GaneshChaturthiQuiz.css';
...
function GaneshChaturthiQuiz({ onBack }) {
  ...
  const loadQuestions = useCallback(async () => {
    const ganeshQuestionIds = ['ganesh1', 'ganesh2', 'ganesh3', 'ganesh4', 'ganesh5', 'ganesh6', 'ganesh7', 'ganesh8', 'ganesh9', 'ganesh10'];
    
    try {
      console.log('Loading Ganesh Chaturthi questions...');
      const questionPromises = ganeshQuestionIds.map(id => 
  ...
  if (loading) {
    return <div className="ganesh-quiz-loading">Loading Ganesh Chaturthi quiz...</div>;
  }

  if (questions.length === 0) {
    return <div className="ganesh-quiz-loading">No questions available. Please try again later.</div>;
  }

  if (showResult) {
    return (
      <div className="ganesh-quiz-result">
        <div className="result-content">
          <h1>🐘 Ganesh Chaturthi Quiz Complete!</h1>
          <div className="score-display">
            <span className="score">{score}</span>
            <span className="total">/ {questions.length}</span>
          </div>
          <p className="score-message">
            {score >= 8 ? "Ganpati Bappa Morya! You're a Ganesh Chaturthi expert!" :
             score >= 6 ? "Excellent! You know your Ganesh traditions well." :
             score >= 4 ? "Good knowledge! Keep learning about Lord Ganesha." :
             "Keep exploring! There's so much to learn about Ganesh Chaturthi."}
          </p>
  ...
  if (!currentQuestion) {
    return <div className="ganesh-quiz-loading">Loading question...</div>;
  }

  return (
    <div className="ganesh-chaturthi-quiz">
      ...
      <div className="question-container">
        {currentQuestion.imageId && (
          <div className="question-image">
            <img src={currentQuestion.imageId} alt="Ganesh Chaturthi" />
          </div>
        )}
...
export default GaneshChaturthiQuiz;
